...
# Vision Transformer (ViT) integration for content extraction
class ViTContentExtractor(nn.Module):
    def __init__(self, image_size=256, patch_size=32, num_classes=1000, dim=1024, depth=6, heads=16, mlp_dim=2048, dropout=0.1, emb_dropout=0.1):
        super(ViTContentExtractor, self).__init__()
        self.vit = ViT(
...
def run_style_transfer(cnn, normalization_mean, normalization_std, content_img, style_img, input_img, num_steps=300, style_weight=100000000, content_weight=10):
    ...
    while run[0] <= num_steps:
        def closure():
            ...
            if run[0] % 50 == 0:
                ...
                plt.figure()
                imshow(input_img, title='Output at Step {}'.format(run[0]))
                print()
